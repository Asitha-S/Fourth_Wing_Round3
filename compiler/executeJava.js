...
const executeJava = (filepath, input = '', timeoutMs = 5000) => {
    ...
    // Pre-processing: Remove 'public' from class definition to avoid filename mismatch errors
    try {
        let content = fs.readFileSync(filepath, 'utf8');
        // Replace 'public class' with 'class' to allow execution even if filename doesn't match
        if (content.includes('public class')) {
            content = content.replace(/public\s+class/g, 'class');
            fs.writeFileSync(filepath, content);
        }
    } catch (e) {
        console.error("Error patching Java file:", e);
    }
